...
class SessionBuilder {
  ...
  async initOutgoing(device, onlyMemory = false, forceNewSession = false) {
    ...
    return await queueJob(fqAddr, async () => {
      ...
      let record = forceNewSession ? null : await this.storage.loadSession(fqAddr, onlyMemory);
      if (!record) {
  ...
  async initIncoming(record, message) {
    ...
    if (message.preKeyId && !preKeyPair) {
      throw new errors.PreKeyError(`Invalid PreKey ID - PreKeyId: ${message.preKeyId}`);
    }
